    ...
    <div class="max-w-7xl mx-auto px-4">
        ...
        <div x-data="{ mobileMenuOpen: false }" x-show="mobileMenuOpen" @click.away="mobileMenuOpen = false" 
            ...
            <div class="px-4 py-2 space-y-1">
                @auth
                    @if(auth()->user()->hasRole('admin'))
                        <a href="{{ route('admin.dashboard') }}" class="block px-4 py-3 rounded-lg font-semibold text-red-700 bg-red-100 hover:bg-red-200 transition flex items-center">
                            <i class="bi bi-speedometer2 mr-3"></i>
                            <span class="mr-2">Admin Dashboard</span>
                            <span class="inline-block bg-gradient-to-r from-red-500 to-red-700 text-white text-xs px-2 py-0.5 rounded-full font-bold uppercase tracking-wider">Admin</span>
                        </a>
                    @else
                        <a href="/" class="block px-4 py-3 rounded-lg font-medium text-gray-700 hover:bg-blue-100 hover:text-blue-700 transition flex items-center {{ request()->is('/') ? 'bg-blue-100 text-blue-700' : '' }}">
                            <i class="bi bi-house mr-3"></i>
                            Home
                        </a>
                        <a href="{{ route('routes.index') }}" class="block px-4 py-3 rounded-lg font-medium text-gray-700 hover:bg-blue-100 hover:text-blue-700 transition flex items-center {{ request()->routeIs('routes.index') ? 'bg-blue-100 text-blue-700' : '' }}">
                            <i class="bi bi-ticket-perforated mr-3"></i>
                            Book Ticket
                        </a>
                        <a href="{{ route('user.bookings') }}" class="block px-4 py-3 rounded-lg font-medium text-gray-700 hover:bg-blue-100 hover:text-blue-700 transition flex items-center {{ request()->routeIs('user.bookings') ? 'bg-blue-100 text-blue-700' : '' }}">
                    ...
                    <div class="border-t border-gray-200 pt-2 mt-2">
                        <a href="{{ route('profile.edit') }}" class="block px-4 py-3 rounded-lg text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition flex items-center">
                            <i class="bi bi-person-circle mr-3 text-blue-500"></i>
                            Profile
                        </a>
                        @if(auth()->user()->hasRole('admin'))
                        <a href="{{ route('admin.dashboard') }}" class="block px-4 py-3 rounded-lg text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition flex items-center">
                            <i class="bi bi-speedometer2 mr-3 text-red-500"></i>
                            Admin Dashboard
                        </a>
                        @else
                        <a href="{{ route('dashboard') }}" class="block px-4 py-3 rounded-lg text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition flex items-center">
                            <i class="bi bi-speedometer2 mr-3 text-blue-500"></i>
                            Dashboard
                        </a>
                        @endif
                        <a href="#" class="block px-4 py-3 rounded-lg text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition flex items-center">
                ...
                @else
                    ...
                    <div class="border-t border-gray-200 pt-2 mt-2">
                        <a href="{{ route('login') }}" class="block px-4 py-3 rounded-lg font-medium text-gray-700 hover:bg-blue-100 hover:text-blue-700 transition flex items-center">
                            <i class="bi bi-box-arrow-in-right mr-3"></i>
                            Login
                        </a>
                        <a href="{{ route('register') }}" class="block px-4 py-3 rounded-lg font-medium text-gray-700 hover:bg-blue-100 hover:text-blue-700 transition flex items-center">
